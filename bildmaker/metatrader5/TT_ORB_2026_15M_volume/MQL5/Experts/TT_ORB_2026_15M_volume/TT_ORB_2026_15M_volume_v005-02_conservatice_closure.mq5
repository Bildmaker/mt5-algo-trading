...
int          g_dailyVolaNoTradePrintDayKey  = 0;
...
int OnInit()
{
      ...
      Print("TT_ORB_2026_15M_volume_v005-02_conservatice_closure initialisiert fuer ", _Symbol,
...
void OnDeinit(const int reason)
{
      ...
      Print("TT_ORB_2026_15M_volume_v005-02_conservatice_closure beendet. Reason=", reason);
}
...
void OnTick()
{
      ...
      DrawSessionRectangles();
      UpdateChartComment();
      MaybePrintDailyVolatilityNoTradeAtNine(currentBarTime);
...
bool GetDailyVolatilityTradeStatus(double &dailyVolatility, string &statusText)
{
      dailyVolatility = 0.0;
      statusText      = "";
      ...
      if(InpDailyVolatilityFilterMode == DAILY_VOLATILITY_FILTER_OFF)
      {
            statusText = "Filter AUS";
            return true;
      }

      if(!GetDailyVolatilityValue(dailyVolatility))
      {
            statusText = "D1 ATR Wert konnte nicht gelesen werden.";
            return false;
      }
      ...
      if(InpDailyVolatilityFilterMode == DAILY_VOLATILITY_FILTER_GREATER_THAN)
      {
            ...
            comparisonText = passed ? "ueber Schwelle" : "unter/gleich Schwelle";
      }
      else if(InpDailyVolatilityFilterMode == DAILY_VOLATILITY_FILTER_LESS_THAN)
      {
            passed = (dailyVolatility < InpDailyVolatilityThreshold);
            comparisonText = passed ? "unter Schwelle" : "ueber/gleich Schwelle";
      }
      else
      {
            statusText = "Unbekannter Daily-Vola Filtermodus.";
            return false;
      }

      statusText = StringFormat("D1 ATR=%.2f | Schwelle=%.2f | Modus=%s | %s",
                                dailyVolatility,
...
{
      string statusText = "";
      const bool passed = GetDailyVolatilityTradeStatus(dailyVolatility, statusText);

      if(!passed && InpVerboseLogging)
      {
            PrintFormat("Daily-Vola Filter blockiert: %s", statusText);
      }

      return passed;
}

color GetTradeWindowColor()
{
      if(InpDailyVolatilityFilterMode == DAILY_VOLATILITY_FILTER_OFF)
            return InpTradeWindowColor;

      double dailyVolatility = 0.0;
      string statusText      = "";

      if(!GetDailyVolatilityTradeStatus(dailyVolatility, statusText))
            return clrFireBrick;

      return InpTradeWindowColor;
}

void MaybePrintDailyVolatilityNoTradeAtNine(const datetime currentBarTime)
{
      if(!IsNewYorkBarTime(currentBarTime, 9, 0))
            return;

      const int nyDayKey = GetNyDayKey(currentBarTime);
      if(g_dailyVolaNoTradePrintDayKey == nyDayKey)
            return;
      ...
      PrintFormat("09:00 NY: Kein Trade findet statt, weil der Daily-Vola D1-ATR Filter blockiert. %s",
                  statusText);

      g_dailyVolaNoTradePrintDayKey = nyDayKey;
}
...
string BuildChartCommentText()
{
      string commentText = "TT_ORB_2026_15M_volume_v005-02_conservatice_closure";
      commentText += "\n";
      commentText += BuildDailyVolatilityCommentLine();

      return commentText;
}

string BuildDailyVolatilityCommentLine()
{
      double dailyVolatility = 0.0;
      string statusText      = "";

      const bool allowed = GetDailyVolatilityTradeStatus(dailyVolatility, statusText);

      if(InpDailyVolatilityFilterMode == DAILY_VOLATILITY_FILTER_OFF)
            return "D1-ATR Filter: AUS";

      return StringFormat("D1-ATR Filter: %s | %s",
                          allowed ? "ERLAUBT" : "BLOCKIERT",
                          statusText);
}
...
{
      return IsNewYorkBarTime(serverBarTime, InpOpeningRangeHour, InpOpeningRangeMinute);
}

bool IsNewYorkBarTime(const datetime serverBarTime,
                      const int      hour,
                      const int      minute)
{
      MqlDateTime nyTime;
      ...
      return (nyTime.hour == hour && nyTime.min == minute);
}
